...
def classify_analyze():
    ...
    if return_type == 0:
        st.error(
            "❌ 사진 분석에 실패했습니다."
            if st.session_state["langpack"] == 0
            else "❌ Photo analysis failed."
        )
    elif return_type == 1:
        korText = f"<{classify_result}> 으로 확인됩니다. 신뢰도는 {round(probability * 100, 2)}% 입니다."
        engText = GoogleTranslator(source="auto", target="en").translate(korText)
        if st.session_state["langpack"] == 0:
            ...
            if classify_result == "반입 가능한 물품":
                ...
                if classify_result == "보조배터리":
                    st.error(
                        "Please check whether you can bring a spare battery on board."
                    )
                else:
                    st.error(f"This item is not permitted carry-on.")
...
st.title(
    body=(
        "자기야!! 이거 :blue[기내반입] 되나? 🤔"
        if st.session_state["langpack"] == 0
        else "Honey! Can I bring this on board? 🤔"
    ),
    width="stretch",
...
uploaded_file = st.file_uploader(
    label=(
        ...
        if st.session_state["langpack"] == 0
        else "Please take or upload a photo of the item. The photo will be deleted immediately after verification."
    ),
...
if st.button(
    (
        "보조배터리 기내반입 가능여부 확인"
        if st.session_state["langpack"] == 0
        else "Check whether you can bring a spare battery on board"
    ),
    on_click=session_change(),
):

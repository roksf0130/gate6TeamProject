...
def azure_cv_ocr():
    ...
    azure_cv_response = requests.post(
        azure_cv_url, headers=azure_cv_headers, data=ocr_image
    )
    ...
    while True:
        result = requests.get(
            operation_url, headers={"Ocp-Apim-Subscription-Key": azure_cv_key}
        )
